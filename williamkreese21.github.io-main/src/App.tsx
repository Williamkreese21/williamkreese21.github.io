...
class AppErrorBoundary extends Component<{ children: ReactNode }, { hasError: boolean }> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <main className="min-h-screen bg-dark text-white flex items-center justify-center px-6 text-center">
          <div>
            <p className="font-mono text-brand uppercase tracking-widest text-sm">System unavailable</p>
            <h1 className="mt-3 text-3xl font-bold">Please reload the page.</h1>
          </div>
        </main>
      );
    }
  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.ERROR, (_event, data) => {
        if (data.fatal) {
          console.warn('Background video unavailable', data.details);
          hls.destroy();
        }
      });
...
function HackerLoader({ onComplete }: Readonly<{ onComplete: () => void }>) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const spinners = ['|', '/', '-', '\\'];

  useEffect(() => {
    if (currentIndex < hackerLines.length) {
      const timer = setTimeout(() => setCurrentIndex(index => index + 1), 220);
  ...
  return (
    <div className="w-full h-full p-8 md:p-16 font-mono text-[11px] md:text-[13px] leading-none text-brand flex flex-col justify-center items-center tracking-[0.25em] uppercase">
      <div className="flex flex-col gap-1.5 w-fit text-left">
        {hackerLines.map((line, index) => {
          return (
            <div key={line} className="flex">
              <span>{line}</span>
              {index < currentIndex && <span className="text-white ml-2">done</span>}
              {index === currentIndex && currentIndex < hackerLines.length && (
                <span className="ml-2">{spinners[currentIndex % spinners.length]}</span>
              )}
            </div>
          );
        })}
        {currentIndex >= hackerLines.length && <div className="text-white">Complete.</div>}
  ...
  return (
    <AppErrorBoundary>
      <>
        ...
        {showLoader && (
          <div
            className={`fixed inset-0 z-[9999] bg-dark flex flex-col transition-opacity duration-500 pointer-events-none ${
              appLoaded ? 'opacity-0' : 'opacity-100'
            }`}
          >
